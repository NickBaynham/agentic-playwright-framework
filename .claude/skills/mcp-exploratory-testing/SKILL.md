...
- Optional: `keep-browser-open` flag to suppress the default browser-close step at the end of the session.
...
23. Capture locator candidates for meaningful user-facing elements and workflow-critical controls.
24. Record locator confidence and rationale for every candidate.
25. Prefer locator candidates in this order: role/name, label, placeholder, text, test id, stable CSS, XPath last.
26. Treat locator candidates as exploration evidence, not final automation implementation decisions.
27. Capture element relationships when locators depend on context, such as product cards, table rows, menus, repeated buttons, dialogs, and lists.
28. Record locator risks for dynamic, repeated, hidden, conditional, or ambiguous elements.
29. Separate application behavior observations from locator/tooling observations.
...
- Close the browser with `browser_close` after the session report (and any split reports or BDD/execution artifacts produced by a related workflow) is written. This is the default behavior.
- Suppress the close only when the user has supplied the `keep-browser-open` flag in the invoking command's arguments, or has otherwise explicitly asked to keep the browser open in the conversation.
- When the close is suppressed, record under Tooling Notes that the browser was intentionally left open at the user's request, including the reason if one was given.
- The close step runs after the Final Review Checklist passes. If the checklist fails, fix the report before closing the browser.
- If `browser_close` itself reports a tooling problem, record it as a Tooling Behavior anomaly and proceed; do not retry silently.
- This rule applies to every workflow defined below (Bounded Workflow Exploration, App Reconnaissance, Exploration Review when MCP was used, and Exploration to BDD Handoff when MCP was used).

## Locator Candidate Capture Rules

During exploration, collect locator candidates for meaningful elements that may be used later in automation. These candidates are evidence, not final implementation decisions.
...
Locator candidates must not be treated as final page object implementation decisions. Final locator decisions are made by the `agentic-playwright-automation` skill during page object creation.

## Repeated and Dynamic Element Rules

When elements repeat — product cards, table rows, search results, menu items, repeated buttons — or appear only under specific state:

1. Capture the repeated element group.
2. Identify the parent-child relationship (e.g., "Add to cart" button inside a product card).
3. Prefer scoped locator strategies (locate the parent, then find the action inside it).
4. Record whether a per-instance data-test attribute exists.
5. Record risks if a naïve locator would click the first matching element globally.
...
Exploration reports must include an Automation Handoff Notes section summarizing:

- recommended page models
- locator candidates to review
- locator risks
- repeated/dynamic element notes
- data dependencies that affect locators
- tool interaction notes (e.g., MCP click fallbacks, snapshot blind spots)

The Automation Handoff Notes section is the primary input that the `exploratory-to-bdd` and `agentic-playwright-automation` skills consume.
...
10. Close the browser with `browser_close` per the Browser Lifecycle Rules, unless the `keep-browser-open` flag was supplied or the user explicitly asked to keep the browser open. If suppressed, record this in Tooling Notes.
...
7. Close the browser with `browser_close` per the Browser Lifecycle Rules, unless the `keep-browser-open` flag was supplied or the user explicitly asked to keep the browser open. If suppressed, record this in Tooling Notes.
...
- The browser was closed with `browser_close` after the report was finalized, unless the `keep-browser-open` flag was supplied or the user explicitly asked to keep the browser open; if suppressed, Tooling Notes records that.
- Locator candidates were captured for meaningful workflow elements, with confidence and rationale.
- Repeated and dynamic element risks are documented where applicable.
- Locator candidates are clearly marked as non-final.
- An Automation Handoff Notes section is present.
